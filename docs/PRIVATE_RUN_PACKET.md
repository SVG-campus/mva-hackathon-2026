...
- The initial phenotype-ablation control used an empty `phenotypicFeatures` list, which Exomiser correctly rejected. The repaired control uses the HPO ontology root `HP:0000001` as a deliberately non-specific feature. Completed main-route outputs are reused only when all three expected non-empty files are present; partial control output is archived before retry.
- The first generated CSV preserved Exomiser's bare chromosome labels (`1`, `2`, etc.), while the frozen challenge validator requires `chr`-prefixed GRCh38 labels. Structural validation rejected all 20 chromosome fields before submission. The reducer now adds the prefix deterministically and is rerun on the unchanged ten-pair ranking.
- The first upload used the VCF sample identifier as `proband_id`, but the deployed backend accepts only the public template identifier `PROBAND01`. The Space rejected the file with a scoring error before recording an entry; the live quota remained 0/6. Packaging now maps to the official identifier explicitly, and the local validator has a regression gate for it.
